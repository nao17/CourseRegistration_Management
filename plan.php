...
<header>
	<h1>目標の編集</h1>
      ...
      新規で目標を<a href="plan_new.php">立てる</a>
  </header>
  <?php
  //遷移したら最初にidを格納
...
//whileループを閉じる
 ...
 ?>
  ...
  <?
    ...
    $stmhI = $pdo->prepare($sql);
    $stmhI->bindValue(":name", "$Str_Name");
    $stmhI ->execute();

    echo "<table>";
    $ArrayDisplay= array('学年', '学期');
  ...
  while($rowsI = $stmhI->fetch(PDO::FETCH_ASSOC)){
    echo "<tr>";
    echo "<td>". $rowsI['YearT']. "</td>";
    echo "<td>". $rowsI['termT']. "</td>";
    echo "</tr>";
}
echo "</table>";
